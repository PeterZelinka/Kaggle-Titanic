...
## Introduction
This project represents a take on the Kaggle Titanic competition. To predict the survived column Logistic regression, K-neighbors Classifier and Random Forrest Classifier are trained on the training data. Each model together with various hyperparameters is input to sklearn's grid search and model with highest accuracy is returned. The workflow of the project was desinged such that it is relatively easy add/remove models and/or parameters.
